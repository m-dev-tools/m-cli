...
- [ ] **LSP Stage 4 (optional polish), driven by self-trial signal:** workspace configuration beyond `m-cli.*` (per-rule disable, severity overrides), hover (rule descriptions), completion (M command keywords from `standard_commands()`). Wait until real-use friction surfaces what to prioritise.
