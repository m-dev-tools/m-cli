...
| 2 | 1 | Linter (logic) | MAJOR | 🟡 Partial | [`m lint`](#62-m-lint) | 40 of XINDEX's 66 rules implemented (37 single-file + 3 cross-routine: M-XINDX-007 undefined-routine, M-XINDX-008 undefined-label, M-XINDX-049 unused-label). Cross-routine rules opt in via `Rule.needs_workspace=True`; the CLI builds a `WorkspaceIndex` once and passes it through. Remaining ~26 rules require data-flow / scope tracking. |
