...
| 7 | 2 | Coverage | MAJOR | 🟡 Two slices shipped | `m coverage` (Phase C) | Runner uses YDB's built-in `view "TRACE"` (one pass per run, no per-label ZBREAK overhead). Label-level: 85/123 = 69.1% on m-tools, byte-identical to `ycover`. Line-level data is currently label-granular (every executable line of a covered label reports hit). Output: `text` (default), `text --lines`, `json`, `lcov` (genhtml / Codecov / Coveralls). True per-line counts await decoding YDB's TRACE offset semantics. |
